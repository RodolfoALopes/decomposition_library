...
void Benchmarks::setMinX(double inVal){
    minX = inVal;
}
void Benchmarks::setMaxX(double inVal){
    maxX = inVal;
}
...
double Benchmarks::rastrigin(double*x,int dim){
    ...
    for(i = dim - 1; i >= 0; i--) {
        sum += x[i] * x[i] - 10.0 * cos(2 * CONST_PI * x[i]) + 10.0;
    }
...
double Benchmarks::rastrigin(double *x, int dim, int k)
{
    ...
    for(i=dim/k-1;i>=0;i--)
    {
        result+=x[Pvector[dim/k+i]]*x[Pvector[dim/k+i]]-10.0*cos(2*CONST_PI*x[Pvector[dim/k+i]])+10.0;
    }
    return(result);
}
...
double Benchmarks::ackley(double*x,int dim){
    ...
    for(i = dim - 1; i >= 0; i--) {
        sum1 += (x[i] * x[i]);
        sum2 += cos(2.0 * CONST_PI * x[i]);
    }

    sum = -20.0 * exp(-0.2 * sqrt(sum1 / dim)) - exp(sum2 / dim) + 20.0 + CONST_E;
    return(sum);
}
...
double Benchmarks::ackley(double *x, int dim, int k)
{
    ...
    for(i=dim/k-1;i>=0;i--)
    {
        ...
        sum2+=cos(2.0*CONST_PI*x[Pvector[dim/k+i]]);
    }

    result=-20.0*exp(-0.2*sqrt(sum1/(dim/k)))-exp(sum2/(dim/k))+20.0+CONST_E;
...
double Benchmarks::rot_rastrigin(double *x,int dim,int k)
{
    ...
    for(i=dim-1;i>=0;i--)
    {
        result+=anotherz1[i]*anotherz1[i]-10.0*cos(2*CONST_PI*anotherz1[i])+10.0;
    }
    return(result);
}
...
double Benchmarks::rot_ackley(double *x,int dim,int k)
{
    ...
    for(i=dim-1;i>=0;i--)
    {
        ...
        sum2+=cos(2.0*CONST_PI*anotherz1[i]);
    }
    result=-20.0*exp(-0.2*sqrt(sum1/dim))-exp(sum2/dim)+20.0+CONST_E;
    return(result);
}
